...
public sealed class InputManager
{
    ...
    public void RegisterMouseInput<T>(IWidget widget, InputType inputType, EventHandler<T> action)
        where T : InputEvent
    {
        if (
            inputType
    ...
    /// <param name="action">The event handler to invoke when the input is detected.</param>
    public void RegisterKeyboardInput<TTarget>(
        TTarget widget,
        InputType inputType,
        List<ConsoleKeyEx> keyboardKey,
        EventHandler<KeyboardEvent> action
    )
        where TTarget : IFocusableWidget =>
        RegisterKeyboardInput<TTarget, KeyboardEvent>(widget, inputType, keyboardKey, action);
    ...
    public void RegisterKeyboardInput<TTarget, TEvent>(
        TTarget widget,
        InputType inputType,
        List<ConsoleKeyEx> keyboardKey,
    ...
    public void RegisterInput(
        ...
        List<ConsoleKeyEx>? key = null
    )
    {
        _inputs[inputType] ??= [];
        _inputs[inputType].Add(new InputHandler(widget, action, key));
    }
        ...
        foreach (var (inputType, handlers) in _inputs)
        {
            ...
            foreach (var handler in handlers)
            {
                switch (inputType)
                {
                    ...
                    or InputType.MouseDoubleClick
                    or InputType.MouseClickOut
                    or InputType.MouseEnter
                    or InputType.MouseLeave
                    or InputType.MouseHover
                    or InputType.MouseScroll
                    or InputType.MouseDrag:
                        HandleMouse(handler.Widget, handler.Action, inputType);
                        break;
                    ...
                    or InputType.KeyboardCombination:
                        if (
                            handler.Widget is IFocusableWidget focusableWidget
                            && focusableWidget.IsFocused
                        )
                            HandleKeyboard(handler.Widget, handler.Action, handler.Keys, inputType);
                        break;
                }
            }
        }
    ...
    public void RemoveInput(IWidget widget, InputType? inputType = null)
    {
        ...
        {
            var input = _inputs.FirstOrDefault(x => x.Value.Any(y => y.Widget == widget));
            if (input.Value == null)
                return;
    ...
    private void HandleKeyboard(
        IWidget widget,
        EventHandler<InputEvent> action,
        List<ConsoleKeyEx>? keyboardKeys,
        InputType inputType
    )
    {
    ...
    )
    {
        ...
        return key switch
        {
            _ when inputType == InputType.Keyboard && isTargetKey => new KeyboardEvent(
                key.Key,
                key.Type,
                key.KeyChar
            ),
            ...
            _
                when inputType == InputType.KeyboardPressed
                    && isTargetKey
                    && key.Type == KeyEvent.KeyEventType.Make => new KeyboardPressedEvent(
                key.Key,
                key.Type,
                key.KeyChar
            ),
            ...
            _
                when inputType == InputType.KeyboardCombination
                    && keyboardKeys.All(KeyboardEx.IsKeyBeingPressed) =>
                new KeyboardCombinationEvent(keyboardKeys),
            _ => null,
        };
    }
    ...
    private InputEvent? GetMouseEvent(IWidget widget, InputType inputType)
    {
        ...
        return mousePosition switch
        {
            ...
            _ when inputType == InputType.MouseClick && MouseEx.MouseClicked && isInside =>
                new MouseClickEvent(MouseManager.MouseState, mousePosition),
            _
                when inputType == InputType.MouseDoubleClick
                    && (DateTime.Now - _lastMouseClick).TotalMilliseconds < 500 =>
                new MouseDoubleClickEvent(MouseManager.MouseState, mousePosition),
            _ when inputType == InputType.MouseClickOut && MouseEx.MouseClicked && !isInside =>
                new MouseClickOutEvent(MouseManager.MouseState, mousePosition),
