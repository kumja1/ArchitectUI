...
internal partial class BindableGenerator
{
    ...
    )
    {
        foreach (PropertyInfo propertyInfo in classInfo.Properties)
        {
            ...
            source.WriteLine($"get => GetProperty<{propertyInfo.Type}>({backingFieldName})!;");
            source.WriteLine($"set => SetProperty(nameof({backingFieldName}), value);");
        ...
        [
            .. classInfo.Properties.Select(p =>
                (
                    supportsPartialProps ? p.Name : ToAlpha(p.Name),
    ...
    )
    {
        ...
        source.WriteLine("{");
        source.Indent++;
        foreach (var (_, propertyBackingField, _) in propertyNames)
        {
            cancellationToken.ThrowIfCancellationRequested();
            source.WriteLine(@$"""{propertyBackingField}"" => (T)(object){propertyBackingField},");
        }
        source.WriteLine(
    ...
    private static string ToAlpha(string value) => char.ToUpperInvariant(value[0]) + value[1..];
}
